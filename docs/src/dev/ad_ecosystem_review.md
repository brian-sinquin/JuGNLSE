...
agrees with the published optimum on the soliton order to 0.25 %. It verifies
that the objective is finite across the whole search range before trusting a
root inside it — an earlier version did not, and reported the order at which the
solver diverges as though it were a zero of the derivative.
