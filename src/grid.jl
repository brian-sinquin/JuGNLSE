...
function create_grid(N::Int, time_window::Real, center_wavelength::Real)
    ...
    # Time grid
    dt = time_window / N
    t = range(-time_window/2, time_window/2 - dt, length=N) |> collect
...
# See Also
- [`create_grid`](@ref): Direct grid creation with explicit parameters
"""
function create_grid_from_medium(N::Int, medium::Medium; time_window::Union{Real,Nothing}=nothing)
