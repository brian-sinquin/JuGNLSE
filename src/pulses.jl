...
# Arguments
  ...
  - `grid::Grid`: Time-frequency grid
  - `duration::Real`: Pulse duration [s] - interpretation depends on `T0` flag
  - `power_peak::Real`: Peak power [W]
  - `T0::Bool`: If `true`, `duration` is 1/e half-width T₀; if `false` (default), `duration` is FWHM
  - `time_offset::Real`: Time offset from center [s] (default: 0.0)
  - `phase::Real`: Initial phase [rad] (default: 0.0)
  - `chirp::Real`: Linear chirp parameter (default: 0.0)
...
The `T0` flag controls how `duration` is interpreted:

  - **`T0=false` (default)**: `duration` is the Full-Width at Half-Maximum (FWHM) of intensity

      + Conversion to 1/e amplitude width: T₀ = FWHM / (2√ln2) ≈ FWHM / 1.665

  - **`T0=true`**: `duration` is the 1/e amplitude half-width T₀
...
```julia
# Standard Gaussian pulse (FWHM)
pulse = gaussian_pulse(grid, 100e-15, 5000.0)
# 100 fs FWHM, 5 kW peak

# Gaussian with T0 specification
pulse = gaussian_pulse(grid, 60e-15, 2000.0; T0=true)
```
...
# Notes
  ...
  - Gaussian pulses have no wings (decay faster than sech)
